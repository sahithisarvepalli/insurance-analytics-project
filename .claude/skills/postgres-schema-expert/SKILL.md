...
| `gender` | `VARCHAR(1)` | `'M'` / `'F'` (the default synthetic generator produces only these two values; `'U'` is accepted by the schema but not generated by `src/generate_synthetic.py`) |
