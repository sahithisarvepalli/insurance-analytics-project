...
def run_transform():
    eng = get_engine()
    q = (
        "SELECT c.claim_id, c.member_id, c.provider_id, c.service_date,
"
        "       c.billed_amount, c.allowed_amount, c.paid_amount, c.place_of_service,
"
        "       c.diagnosis_code, c.procedure_code,
"
        "       m.dob, m.region AS member_region, m.effective_date,
"
        "       p.in_network
"
...
"
    ...
    df['age_band'] = pd.cut(df['age'], bins=[0,18,30,45,60,200], labels=['0-18','19-30','31-45','46-60','60+'])

    # KPIs
    kpis = (df
            .groupby(['age_band','member_region','in_network'], dropna=False)
    ...
    monthly = (df
               .groupby(['month','member_region','in_network'])
               .agg(claims=('claim_id','count'), paid_total=('paid_amount','sum'))
               .reset_index())

    # Persist to outputs as CSV
