...
def gen_members(n=50_000, seed=42):
    ...
    term = np.where(
        rng.random(n) < 0.1,
        eff + pd.to_timedelta(rng.integers(30, 730, size=n), unit="D"),
        pd.NaT,
    )
    return pd.DataFrame(
        {
    ...
    return pd.DataFrame(
        {
            "specialty": rng.choice(
                ["PCP", "Cardiology", "Ortho", "Derm", "Oncology"],
                size=n,
    ...
    return pd.DataFrame(
        {
            ...
            "diagnosis_code": rng.choice(
                ["I10", "E11", "M54", "J06", "C50", "Z00"],
                size=n,
                p=[0.25, 0.2, 0.2, 0.2, 0.05, 0.1],
            ),
            "procedure_code": rng.choice(
                ["99213", "99214", "93000", "71020", "80050", "J9271"], size=n
            ),
            "billed_amount": billed,
            "allowed_amount": allowed,
            "paid_amount": paid,
            "place_of_service": rng.choice(
                ["Office", "Inpatient", "Outpatient", "ER"],
                size=n,
                p=[0.6, 0.1, 0.25, 0.05],
            ),
        }
