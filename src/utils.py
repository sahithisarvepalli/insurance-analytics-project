...
def load_config(path: str = 'config/db.yaml') -> dict:
    cfg = {}
    if os.path.exists(path):
        ...
        raw = {}
    # Support env var expansion like ${VAR}
    def expand(v):
        if isinstance(v, str) and v.startswith('${') and v.endswith('}'):
            return os.getenv(v[2:-1])
        return v
    cfg = {k: expand(v) for k, v in raw.items()}
    return cfg
...
def get_engine():
    ...
    if not url:
        # Construct from parts or fall back to sqlite
